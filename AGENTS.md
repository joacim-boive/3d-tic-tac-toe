...
- **Logic self-tests** run without a browser via `tsx`: `npm run check:win`, `check:pick`, `check:ai`, `check:room`, `check:notify`, `check:drop`, `check:selfplay`. Useful for validating game/AI/room-code/notify logic quickly.
