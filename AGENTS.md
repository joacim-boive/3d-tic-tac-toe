...
- **Logic self-tests** run without a browser via `tsx`: `npm run check:win`, `check:pick`, `check:ai`, `check:room`, `check:notify`, `check:drop`, `check:selfplay`, `check:prefs`. Useful for validating game/AI/room-code/notify logic quickly.
- **Balance eval**: `npm run eval:selfplay` runs AI-vs-AI batches and prints first/second win rate, draws, average length, and common openings. Prefer `--difficulty medium` for large N; Hard α-β is for small samples (`--budget Infinity`). To measure Extreme vs Hard (browser think-times): `--difficulty extreme --vs hard --swap --games 40`. Impossible vs Extreme: `--difficulty impossible --vs extreme --swap --games 24`.
