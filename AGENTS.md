...
- **Balance eval**: `npm run eval:selfplay` runs AI-vs-AI batches and prints first/second win rate, draws, average length, and common openings. Prefer `--difficulty medium` for large N; Hard α-β is for small samples (`--budget Infinity`). To measure Extreme vs Hard: `--difficulty extreme --vs hard --swap --games 40 --budget 700`.
